...
export default function Features() {
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div>
        {
          mainFeatures.map(({ id, title }) => <CheckBox id={id} title={title} register={register} key={id} />)
        }
      </div>
      ...
      <div>
        <div>
          ...
          <div>
          {
            nearByFacilities.map(({ id, title }) => <CheckBox id={id} title={title} register={register} key={id} />)
          }
          </div>
        </div>
        <div>
          ...
          <div>
          {
            houseFeatures.map(({ id, title }) => <CheckBox id={id} title={title} register={register} key={id} />)
          }
          </div>
        </div>
        <div>
