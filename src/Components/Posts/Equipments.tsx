...
export default function Equipments() {
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
      <button type="button" className="self-start add-new-back-btn"  onClick={() => handleProcedureClick("特色")}>上一步</button>
      <h3 className="add-new-title">設備</h3>
      <div>
        {
          equipments.map(({ id, title }) => <CheckBox id={id} title={title} register={register} key={id} />)
        }
      </div>
