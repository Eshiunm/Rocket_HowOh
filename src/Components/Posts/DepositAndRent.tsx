...
export default function DepositAndRent() {
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
      <button type="button" className="self-start add-new-back-btn"  onClick={() => handleProcedureClick("雜支")}>上一步</button>
      <h3 className="add-new-title">訂金與租金</h3>
      <div className="add-new-input-block" >
        <label htmlFor="rent" className="text-sm">每月租金</label>
        <input type="text" id="rent" className="add-new-input w-1/3" placeholder="租金" {...register("rent", { required: true })} />
      </div>
      <div className="add-new-input-block" >
        <label htmlFor="deposit" className="text-sm">押金</label>
        <select id="deposit" className="add-new-input w-1/3" {...register("deposit", { required: true })}>
